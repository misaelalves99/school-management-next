    ...
    <StudentsContext.Provider value={contextValue}>
      <ul data-testid="student-list">
        {contextValue.students.map((s) => (
          <li key={s.id ?? s.name}>{s.name}</li> // fallback para evitar erro "never"
        ))}
      </ul>

      <button
        onClick={() =>
          contextValue.addStudent({
            name: 'Novo Aluno',
      ...
      <button
        onClick={() => {
          ...
          if (first && first.id != null) {
            contextValue.updateStudent(first.id, { ...first, name: 'Aluno Atualizado' });
          }
        }}
      >
        Atualizar
      ...
      <button
        onClick={() => {
          const first = contextValue.students[0];
          if (first && first.id != null) {
            contextValue.deleteStudent(first.id);
          }
        }}
      >
        Deletar
...
describe('StudentsContext', () => {
  ...
  it('retorna estudante pelo ID', () => {
    ...
    expect(studentList.length).toBe(mockStudents.length);
